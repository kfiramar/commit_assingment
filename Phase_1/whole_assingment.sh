...
KFIR_SUBNET1_NAME="KfirDbSubnet1"
KFIR_SUBNET2_NAME="KfirDbSubnet2"
KFIR_VPC_CIDR="10.0.0.0/16"
KFIR_SUBNET1_CIDR="10.0.1.0/24"
KFIR_SUBNET2_CIDR="10.0.2.0/24"
KFIR_ECS_CLUSTER_NAME="KfirEcsCluster"
...
deploy_ecs_service() {
    ...
    # Check if the Target Group ARN is retrieved
    if [ -z "$tg_arn" ]; then
        echo "Target Group ARN not found. Ensure the target group 'kfir-tg' exists."
        exit 1
    fi

    # Specify the subnets and security groups for the network configuration
    local subnets=$(get_subnet_id $KFIR_SUBNET1_NAME),$(get_subnet_id $KFIR_SUBNET2_NAME)
    ...
    aws ecs create-service --cluster $KFIR_ECS_CLUSTER_NAME \
        ...
        --task-definition $KFIR_ECS_CLUSTER_NAME \
...
# Phase 1g: Service exposed to the Internet over HTTPS
configure_https() {
    echo "Configuring HTTPS..."
    # Generate self-signed certificate
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout kfir-nginx.key -out kfir-nginx.crt -subj "/C=US/ST=YourState/L=YourCity/O=YourOrganization/CN=yourwebsite.com"
    # Note: Configure the load balancer or web server to use kfir-nginx.crt and kfir-nginx.key
}
